...
import {
  Capacity_Validation,
  Discount_Validation,
  Facilities_Validation,
  RoomNumber_Validation,
  RoomPrice_Validation,
...
export default function RoomsData() {
  ...
  const onSubmit = async (values: IRoomSubmit) => {
    try {
      ...
      Object.entries(values).forEach(([key, value]) => {
        if (key === "facilities") {
          values[key].forEach((faciliy) =>
            formData.append("facilities[]", faciliy)
          );
        } else {
      ...
      const { data } = roomId
        ? await privateUserAxiosInstance.put(
            ROOMS_URLS.UPDATE_ROOM(roomId),
            formData
          )
        : await privateUserAxiosInstance.post<IRoomCreation>(
  ...
  const getAllFacilities = async () => {
    try {
      const { data } =
        await privateUserAxiosInstance.get<IFacilities_Room_Response>(
          ROOMS_URLS.GET_FACILITIES_ROOM,
          {
            params: {
              page: 1,
              size: 1000,
    ...
    (async () => {
      ...
      if (roomId) {
        try {
          setIsLoading(true);
          const { data } =
            await privateUserAxiosInstance.get<IRoom_Id_Response>(
              ROOMS_URLS.GET_ROOM(roomId)
            );
          ...
          setValue(
            "facilities",
            room?.facilities.map((facility) => facility._id)
          );
          ...
          {loading ? (
            ...
            <form onSubmit={handleSubmit(onSubmit)} style={{ width: "100%" }}>
                ...
                >
                  <CustomInput
                    ...
                    rules={{
                      ...
                      validate: {
                        isPositive: (value) =>
                          Number(value) > 0 || t("room.PricePositive"),
                      },
                    }}
              ...
              <Box
                ...
                sx={{
                  border:
                    theme === "light" ? "1px dotted rgba(0, 146, 71, 1)" : "1px dotted rgba(50, 82, 223, 1)",
                  height: "140px",
                  backgroundColor:
                    theme === "dark" ? "black" : "rgba(241, 255, 240, 1)",
                  display: "flex",
                  justifyContent: "center",
                  alignItems: "center",
                  mt: "20px",
                  ...
                  <Button
                    ...
                    sx={{
                      ...
                      "&:hover": {
                        ...
                        border: "1px solid rgba(32, 63, 199, 1)",
                      },
                    }}
                    type="submit"
                  >
                    {isSubmitting ? t("room.Editing") : t("room.EditRoom")}
                  </Button>
                ) : (
                  <Button
                    variant="contained"
                    ...
                    type="submit"
                  >
                    {isSubmitting ? t("room.Adding") : t("room.AddRoom")}
                  </Button>
                )}
              </Box>
